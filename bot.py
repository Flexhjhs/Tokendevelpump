...
# Payment options - now with 7 choices including token creation
PAYMENT_OPTIONS = {
...
async def contract_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Check if it's a new token creation request
        is_new_token = contract.lower() == "new"
        
        # Create keyboard with payment options
        keyboard = []
        for opt_id, opt in PAYMENT_OPTIONS.items():
            # For new tokens, only show option7 (token creation service)
            if is_new_token and opt_id != "option7":
                continue
            # For existing tokens, show all options except token creation
...
async def payment_option(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Add special note for token creation
    note = ""
    if option == "option7":
        note = "\n\n💎 Includes:\n- Token creation\n- Website deployment\n- Social media setup\n- Whitepaper template"
...
async def confirm_payment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Add special monitoring for token creation
    monitor = ""
    if option == "option7":
        monitor = (
            "\n🔧 Creation Progress:\n"
